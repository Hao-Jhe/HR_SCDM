...
use  time_manager_mod, only: time_type, get_time, Time_for_hs, get_date    ! zw
...
!-----------------------------------------------------------------------------------
   ...
   character(len=256) :: drag_file='kf_1'            ! Name of drag file
...
!
   ...
   real :: sigma_strat1=0.15,sigma_strat2=0.095 ! levels defining transition from tka to tk_strat tjr
...
!-----------------------------------------------------------------------
   ...
   real :: tka, tks      ! vkf(128,64)   ! vkf 
   real :: scdamp
   real :: trdamp, twopi
   real :: tk_strat
   real, allocatable, dimension(:,:) :: vkf   ! zw
...
!#######################################################################
 ...
 subroutine hs_forcing ( is, ie, js, je, dt, Time, lon, lat, p_half, p_full, &
                         ...
                         s_geo, & !mj
                         count_int, teq_hs_init, tau_hs_init, & ! zw
                         mask, kbot )
...
!-----------------------------------------------------------------------
 ...
 type(time_type), intent(in)                  :: Time
      ...
      integer, intent(in)                        :: count_int   ! zw
      real, intent(inout), dimension(:,:,:,:)   :: teq_hs_init  ! zw
      real, intent(inout), dimension(:,:,:,:)   :: tau_hs_init  ! zw
...
!-----------------------------------------------------------------------
   ...
   integer :: n_ozone, n_cold_tracer  !hjh
   logical :: used
   real    :: flux, sink, value
   character(len=128) :: scheme, params
   real, dimension(size(t,1),size(t,2))           :: vkf_tmp
   integer :: my_pid, my_mpid
   character(4) :: pid_str
   integer :: lat_numb

   lat_numb = size(lon,2)*mpp_npes()
...
!     surface pressure
     ...
     if (present(kbot)) then
      ...
      !& 'test/' // pid_str // '_kfvkf.txt')
      !     write(my_pid,*) "size_kf= ",size(kf,1),size(kf,2)
      !     write(my_pid,*) "size_vkf= ",size(vkf,1),size(vkf,2)
      !close(my_pid) 
...
!     thermal forcing for held & suarez (1994) benchmark calculation
      call newtonian_damping ( Time, lat, ps, p_full, p_half, t, ttnd, teq, tau, count_int, teq_hs_init, tau_hs_init, mask,surface_forcing )
      ...
      !vkf_tmp = vkf(:,mpp_pe()*2+1:(mpp_pe()+1)*2)
      vkf_tmp = vkf(:,mpp_pe()*size(lon,2)+1:mpp_pe()*size(lon,2)+2)
      ...
      if (id_vkf > 0) used = send_data ( id_vkf, vkf_tmp, Time, is, js)
...
!     -------- tracers -------
      ...
      n_ozone = get_tracer_index(MODEL_ATMOS, 'ozone')  !hjh
      n_cold_tracer = get_tracer_index(MODEL_ATMOS, 'cold_tracer')  !hjh
      ...
      if(num_tracers == size(rdt,4)) then
        do n = 1, size(rdt,4)
           ...
           elseif(n == get_tracer_index(MODEL_ATMOS, 'APV')) then !mj Blocking index as Schwierz2004
              ...
              rdt(:,:,:,n) = rdt(:,:,:,n) + rtnd !mj
           elseif(n == n_ozone) then  !hjh
              continue  !hjh
           elseif(n == n_cold_tracer) then  !hjh
              continue  !hjh
           else !mj
...
!-----------------------------------------------------------------------
   ...
   IF (ALLOCATED (vkf)) deallocate(vkf)
   allocate(kf(size(lonb,1)-1,(size(lonb,2)-1)*mpp_npes()))
   allocate(vkf(size(lonb,1)-1,(size(lonb,2)-1)*mpp_npes()))
...
! If negative, damping time units are (days), value is the damping time. It is converted to (1/s)
      ...
      open(10,file=trim(drag_file))
!      open(10,file=trim('/uufs/chpc.utah.edu/common/home/u1006314/my_research/MODEL/WR_simpleGCM-master/data/drag.txt'))
      do ikf=1,128
      ...
      if (k_strat < 0.) then                 ! tjr
        tk_strat = -1./(86400*k_strat)
      else
        tk_strat = k_strat
      endif
      if (kf(1,1) < 0.) then
...
!     ----- register diagnostic fields -----
      ...
      id_vkf = register_diag_field ( mod_name, 'vkf', axes(1:2), Time, &
                      'surface drag', '1/day'   , &
                      missing_value=missing_value, range=(/0.,3./) )
...
!#######################################################################
 ...
 subroutine newtonian_damping ( Time, lat, ps, p_full, p_half, t, tdt, teq, tau, count_int, teq_hs_init, tau_hs_init, mask, surface_forcing )
!-----------------------------------------------------------------------
!
...
!-----------------------------------------------------------------------
     ...
     tstr, sigma, the, tfactr, rps, p_norm, dQdistr
       ...
       type(time_type)  :: Time2                ! zw
       integer          :: idays_init           ! zw
       integer          :: dayforyear=365       ! zw
...
!  ----- compute equilibrium temperature (teq) ----- !mj troposphere
      if(trim(equilibrium_t_option) == 'Held_Suarez' .or. trim(equilibrium_t_option) == 'JFV' .or. trim(equilibrium_t_option) == 'strat_file' ) then
         ...
         call get_time(Time,seconds,days)   ! zw
         t_days_ind = mod(days,365)+1
         if(count_int == 1) then
           do idays_init=1,dayforyear
             call Time_for_hs(Time2,idays_init)
             call get_temp(Time2,p_half,teq)
             teq_hs_init(:,:,:,idays_init)=teq
           enddo
           teq = teq_hs_init(:,:,:,1)
         else
           teq = teq_hs_init(:,:,:,t_days_ind)
         endif
...
!  ----- compute damping ----- !mj this is the troposphere
      ...
      if(trim(equilibrium_tau_option) == 'from_file') then !mj
         ...
         if(count_int == 1) then
           do idays_init=1,dayforyear
             call Time_for_hs(Time2,idays_init)
             call get_tau(Time2, p_half, tdamp)
             tau_hs_init(:,:,:,idays_init)=tdamp
           enddo
           tdamp = tau_hs_init(:,:,:,1)
         else
           tdamp = tau_hs_init(:,:,:,t_days_ind)
         endif 
...
real, dimension(size(u,1),size(u,2)) :: sigma, vfactr, sfactr, rps, vcoeff ! zw
integer :: i,j,k
!real    :: vcoeff                              ! zw
...
!----------------compute damping----------------------------------------
      ...
      my_mpid=mpp_pe()
      !vcoeff(:,:) = -vkf(:,:)/(1.0-sigma_b)             ! zw
      !vcoeff(:,:) = -vkf(:,my_mpid*2+1:(my_mpid+1)*2)/(1.0-sigma_b)             ! zw
      vcoeff(:,:) = -vkf(:,my_mpid*size(u,2)+1:my_mpid*size(u,2)+2)/(1.0-sigma_b)! zw
      rps = 1./ps
      ...
      if (relax_to_specified_wind) then
         do j=1, size(u,2)
            ...
            udt(:,j,k) = (uz(j,k)-umean)*vkf(:,j)
            vdt(:,j,k) = (vz(j,k)-vmean)*vkf(:,j)
         enddo
      else
         ...
         where (sigma(:,:) <= 1.0 .and. sigma(:,:) > sigma_b)
            vfactr(:,:) = vcoeff(:,:)*(sigma(:,:)-sigma_b)      ! zw
            udt(:,:,k)  = vfactr(:,:)*u(:,:,k)
            vdt(:,:,k)  = vfactr(:,:)*v(:,:,k)
         elsewhere
            udt(:,:,k) = 0.0
         ...
         !  write (pid_str, "(I4)") my_pid
         !  open(unit=my_pid,file='/glade2/scratch2/zhengwu/FMS/model/archive/' // &
         !  & 'test/' // pid_str // '_vcoeff_asy.txt')
         !  write(my_pid,*) "vcoeff= ",vcoeff(:,1)
           !write(my_pid,*) "vcoeff= ",vcoeff(:,2)
           !write(my_pid,*) "mpi= ",my_mpid*2+1,(my_mpid+1)*2
         !  close(my_pid) 
         !endif 
         ...
         do j=1,size(r,2)
            do i=1,size(r,1)
               ...
               if(kb.eq.size(r,3))then
                  source(i,j,kb) = max(0.,-vkf(i,j)*(r(i,j,kb)-qsat(i,j,kb))) !mj specific humidity, above sea only
               endif
            enddo
         enddo
      else
...
!------------ surface source and global sink ---------------------------
      ...
      if (present(kbot)) then
         do j=1,size(r,2)
            do i=1,size(r,1)
               kb = kbot(i,j)
               source(i,j,kb) = -vkf(i,j)*(r(i,j,kb)-flux) !mj tracer value fixed to trflux at bottom
            enddo
         enddo
      else
        kb = size(r,3)
