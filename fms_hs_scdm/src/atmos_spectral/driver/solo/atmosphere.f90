...
use       tracer_manager_mod, only: get_number_tracers, get_tracer_index  !hjh

use           ct07_ozone_mod, only: ct07_ozone_init, ct07_ozone  !hjh

use        ozone_forcing_mod, only: ozone_forcing_init, ozone_forcing  !hjh
...
'$Id: atmosphere.F90,v 17.0.2.1 2009/09/17 16:58:15 pjp Exp $'
...
real, allocatable, dimension(:,:,:,:  ) :: vorg
...
real, allocatable, dimension(:,:,:,:  ) :: teq_init        ! zw
real, allocatable, dimension(:,:,:,:  ) :: tau_init        ! zw

real, allocatable, dimension(:,:,:,:  ) :: ozone_init  ! hjh
real, allocatable, dimension(:,:,:,:,:) :: ct07_coeff  ! hjh
logical :: external_ozone  ! hjh
...
allocate (vorg         (is:ie, js:je, num_levels, num_time_levels))
...
allocate (teq_init      (is:ie, js:je, num_levels, 365))        ! zw
allocate (tau_init      (is:ie, js:je, num_levels, 365))        ! zw

allocate (ozone_init (is:ie, js:je, num_levels, 365   ))  ! hjh
allocate (ct07_coeff (is:ie, js:je, num_levels, 365, 9))  ! hjh

p_half = 0.; z_half = 0.; p_full = 0.; z_full = 0.
wg_full = 0.; psg = 0.; ug = 0.; vg = 0.; tg = 0.; vorg = 0.; grid_tracers = 0.
...
!open(unit=my_pid,file='/glade/scratch/zhengwu/FMS/model/output/lat' //&
!  & pid_str // '.txt')
!write(my_pid,'(2(F10.5,2X))') deg_lat(js:je)
!close(my_pid)

! zw
!open(unit=2000,file='/glade/scratch/zhengwu/FMS/model/output/num_levels.txt')
!write(2000,*) "num_levels= ",num_levels
!close(2000)

call hs_forcing_init(get_axis_id(), Time, rad_lonb_2d, rad_latb_2d)

call ct07_ozone_init(get_axis_id(), Time, rad_lonb_2d, rad_latb_2d, external_ozone )  !hjh

call ozone_forcing_init(get_axis_id(), Time, rad_lonb_2d, rad_latb_2d)  !hjh
...
subroutine atmosphere(Time, count_int)
type(time_type), intent(in) :: Time
integer, intent(in)         :: count_int

real    :: delta_t
type(time_type) :: Time_next
real, dimension(size(vorg,1),size(vorg,2),size(vorg,3)) :: vor_prev
real, dimension(size(grid_tracers,1),size(grid_tracers,2),size(grid_tracers,3)) :: input_ozone  !hjh
real, dimension(size(grid_tracers,1),size(grid_tracers,2),size(grid_tracers,3)) :: input_t  !hjh
integer :: n_ozone  ! hjh
...
if(previous.le.1)then
   vor_prev = vorg(:,:,:,previous)
else
   vor_prev = vorg(:,:,:,previous-1)
endif

call hs_forcing(1, ie-is+1, 1, je-js+1, delta_t, Time_next, rad_lon_2d, rad_lat_2d, &
                p_half(:,:,:         ),       p_full(:,:,:           ), &
                    ug(:,:,:,previous),           vg(:,:,:,previous  ), &
                    tg(:,:,:,previous),     vor_prev(:,:,:           ), &
                    grid_tracers(:,:,:,previous,:),                     &
                    ug(:,:,:,previous),           vg(:,:,:,previous  ), &
                    tg(:,:,:,previous),         vorg(:,:,:,previous  ), &
                    grid_tracers(:,:,:,previous,:),                     &
                 dt_ug(:,:,:         ),        dt_vg(:,:,:           ), &
                 dt_tg(:,:,:         ),   dt_tracers(:,:,:,:), & !mj
                 surf_geopotential(:,:), & !mj
                 count_int, teq_init, tau_init)  ! zw

call ct07_ozone(1, ie-is+1, 1, je-js+1, delta_t, Time_next, rad_lon_2d, rad_lat_2d, &
                p_half(:,:,:         ), p_full(:,:,:           ),                   &
                tg(:,:,:,previous),                                                 &
                grid_tracers(:,:,:,previous,:),                                     &
                dt_tracers(:,:,:,:),                                                &
                ozone_init(:,:,:,:  ), ct07_coeff(:,:,:,:,:), count_int )  !hjh

call ozone_forcing(1, ie-is+1, 1, je-js+1, delta_t, Time_next, rad_lon_2d, rad_lat_2d, &
                   p_half(:,:,:         ), p_full(:,:,:           ),                   &
                   tg(:,:,:,previous),                                                 &
                   grid_tracers(:,:,:,previous,:),                                     &
                   dt_tg(:,:,:          ) )  !hjh                                       

! save input ozone from file
  if (external_ozone) then 
    n_ozone = get_tracer_index( MODEL_ATMOS, 'ozone' )  !hjh
    input_ozone = grid_tracers(:,:,:,previous,n_ozone)  !hjh
  endif
...
call spectral_dynamics(Time, psg(:,:,future), ug(:,:,:,future), vg(:,:,:,future), &
                       tg(:,:,:,future), vorg(:,:,:,future), tracer_attributes, grid_tracers(:,:,:,:,:), future, &
...
! replace input ozone in grid_tracer 
  if (external_ozone) then
    grid_tracers(:,:,:,future,n_ozone) = input_ozone  !hjh
  endif
